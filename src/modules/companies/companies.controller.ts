...
export class CompaniesController {
  ...
  @UseGuards(JwtAuthGuard)
  @Post('me/seats')
  addSeat(@Request() req: any, @Body() body: { name: string; email: string; password: string; cardLast4?: string }) {
  ...
  subscribe(
    @Request() req: any,
    @Body() body: { plan: PlanType; cardNumber: string; cardName: string; cardExpiry: string; cardCvv: string }
  ) {
  ...
  @UseGuards(JwtAuthGuard)
  @Post('me/extra-job')
  buyExtraJob(@Request() req: any, @Body() body: { cardLast4?: string }) {
    return this.companiesService.buyExtraJob(req.user.sub, body);
  }
